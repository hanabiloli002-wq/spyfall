...
export default function GameChat() {
  ...
  const { channels, spyNote, isSpy, myRole, players, settings, socketId } = state;
  ...
  // If active tab gets removed, fallback to 'all'
  useEffect(() => {
    ...
    <div className="fixed bottom-4 right-4 z-[100] flex flex-col items-end">
      ...
      {activeTab === 'note' ? (
        ...
        <>
          ...
          <div className={`flex-1 overflow-y-auto p-4 space-y-3 ${tc.bg}`}>
            {messages.length === 0 ? (
              <p className="text-center opacity-50 text-xs mt-4">
                No messages yet...
              </p>
            ) : (
      ...
      <button
        ...
        className={`w-14 h-14 rounded-full flex items-center justify-center shadow-lg transition-transform hover:scale-105 active:scale-95 ${
          isOpen ? 'bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-300' : 'bg-violet-600 text-white shadow-violet-500/30'
        }`}
      >
